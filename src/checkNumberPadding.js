 ...
 */
const checkNoPaddedValues = (arr) => {
    const hasLeadingZero = arr.some(s => checkPaddingForSingleValue(s));
    if (hasLeadingZero)
        return undefined;
 ...
 */
const checkInconsistentPaddingWidth = (arr) => {
 ...
 */
const checkConsistentPadding = (arr, paddedLengths) => {
...
function checkNumberPadding(intStrs) {
    ...
    if (noPadResult !== undefined)
        return noPadResult;
    const padded = arr.filter(s => checkPaddingForSingleValue(s));
